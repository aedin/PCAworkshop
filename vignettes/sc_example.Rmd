...
PCA is the oldest dimension reduction method, and while it is arguably the most popular, it is only one of many possible methods; there are numerous methods related to and adapted from PCA. One such method is **correspondence analysis (CA)**.
...
However, these implementations are not designed for very large data, and thus are unwieldy for single-cell analysis. To enable the fast (and easy) application of CA to such datasets, we developed the `corral` package (which we've been using to plot embeddings, up til this point!).
