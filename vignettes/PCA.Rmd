...
```{r}
sf=2
eigSum.svd= data.frame(rbind(
  D=signif(s$d,sf),
  eigenValues= signif(s$d^2/(nrow(s$u) - 1),sf),
  SD = signif(sqrt(eigenValues),sf),
  Proportion = paste0(signif((eigenValues/sum(eigenValues)),2)*100,"%"),
  Cumulative = paste0(cumsum(signif((eigenValues/sum(eigenValues)),2))*100,"%")))
...
The default version of scale is a zscore standarization
...
Then by extension, a P0, svd of a scaled matrix is the same as svd of a zscore matrix
```{r}
pz<-svd(PZ)
all(pz$v == p0$v)
